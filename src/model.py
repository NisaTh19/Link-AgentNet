## Taken from https://github.com/KarolisMart/AgentNet/blob/main/model.py, extended for link prediction
...
def add_model_args(parent_parser: Optional[ArgumentParser]=None, hyper: bool=False) -> ArgumentParser:
    ...
    parser.add_argument('--dataset', type=str, default='KarateLink', help='Options are [KarateLink, GePhil]')
    parser.add_argument('--iters_per_epoch', type=int, default=30) # Number of steps/batches per epoch. Like in GIN
    parser.add_argument('--n_splits', type=int, default=10, help="Number of splits for StratifiedKFold")
    ...
    parser.add_argument('--readout_type', action='store_true', default='all_steps', help= "Options are ['final_only', 'all_steps', 'all_steps_linear', 'all_steps_mlp']")
...
class LinkPredictionAgentNet(nn.Module):
    def __init__(self, num_features, hidden_units, num_out_classes, dropout, num_steps, num_agents, reduce, node_readout, use_step_readout_lin, 
                ...
                visited_decay=0.9, sparse_conv=False, num_edge_features=0, mean_pool_only=False, edge_negative_slope=0.2, final_readout_only=False):
        
        super(LinkPredictionAgentNet, self).__init__()
        ...
        assert num_agents % 2 == 0, "For link prediction, num_agents must be even."
        self.group_size = num_agents // 2
        ...
        self.num_out_classes = num_out_classes #default is 1 for link prediction (single logit for a "binary" classification)
        ...
        # Agent jump
        self.agent_id_emb = nn.Sequential(
            nn.Linear(self.num_agents, self.dim),
            nn.LeakyReLU(),
        ...
        gate_input_dim = 2   # since we combine two scalars
        self.gate_fc = nn.Linear(gate_input_dim, 1)
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, batch: torch.Tensor, node_pair: torch.Tensor, edge_feat: Optional[torch.Tensor] = None, return_paths: bool = False):

        agent_paths = {}  # <--- Track agent positions over time
        ...
        group_size = self.group_size  # number of agents per target node
        ...
        if edge_emb_sl is not None:
            ...
            edge_index_sl, _ = coalesce(edge_index_sl, None, num_nodes_batch)
            edge_index, _ = coalesce(edge_index, None, num_nodes_batch)
        ...
        # Set agent positions : one group at source node and one group at target node
        source_positions = node_pair[:, 0].view(-1, 1).expand(-1, group_size)   # [batch_size, group_size]
        target_positions = node_pair[:, 1].view(-1, 1).expand(-1, group_size)   # [batch_size, group_size]

        # Concatenate to get positions for all agents for each sample.
        agent_pos = torch.cat([source_positions, target_positions], dim=1)      # [batch_size, total_agents]

        # Flatten for the traversal loop.
        agent_pos = agent_pos.view(-1)  # [batch_size * total_agents]
        if return_paths: # save the first position
            agent_paths[0] = agent_pos.detach().clone()  # Save path snapshot

        # For compatibility with the original code's later operations,
        # we create agent_node and agent_node_attention_value as before.
        agent_node = torch.stack([torch.arange(agent_pos.size(0), device=agent_pos.device), agent_pos], dim=0)  # [2, batch_size * total_agents]
        ...
        for i in range(self.num_steps+1):
            ...
            if i > 0: 
                ...
                if self.random_agent:
                    ...
                    if self.basic_agent and not self.bias_attention:
                        ...
                        if self.num_pos_attention_heads > 1:
                            attn_score = self.attn_lin(attn_score.view(agent_neighbour.size(1), self.num_pos_attention_heads)).squeeze(-1)
                        ...
                        if self.bias_attention and self.basic_agent:
                            ...
                            if self.self_loops:
                                # attn_score[neighbors_visited==1.0] += stay_param # Current node has visited = 1.0
                                attn_score[neighbors_visited==1.0] += stay_param[agent_neighbour[0][neighbors_visited==1.0]]
                            if i > 1: # No previous node at first step
                                # attn_score[previous_visited==1.0] += back_param
                                attn_score[previous_visited==1.0] += back_param[agent_neighbour[0][previous_visited==1.0]]
                ...
                if return_paths:
                    agent_paths[i] = agent_pos.detach().clone()  # Save path snapshot
