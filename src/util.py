## Taken from https://github.com/KarolisMart/AgentNet/blob/main/util.py, added a LinkPredictionDataset
...
def gumbel_softmax(src: Tensor, index: LongTensor, num_nodes: int=0, hard: bool=True, tau: float=1.0, i: int=0):
    ...
    if gumbels.ndim == 2 and gumbels.size(1) == 1:
        gumbels = gumbels.squeeze(-1)
...
class LinkPredictionDataset(torch.utils.data.Dataset):
    def __init__(self, G_nx, k_hop=3, seed=42):
        set_seed(seed)
        ...
        # Create positive edge list (undirected, unique)
        edge_set = set()
        for i, j in G_nx.edges():
            if i < j:
                ...
                edge_set.add((j, i))
        self.positive_edges = [list(e) for e in edge_set]

        # Sample negative edges
        self.negative_edges = self.sample_negative_edges()

        # Compute structural features for the nodes
        ...
        degree_feat = torch.tensor([degree_dict.get(i, 0) for i in range(self.num_nodes)], dtype=torch.float).view(-1, 1)
        clustering_feat = torch.tensor([clustering_dict.get(i, 0.0) for i in range(self.num_nodes)], dtype=torch.float).view(-1, 1)
        betweenness_feat = torch.tensor([betweenness_dict.get(i, 0.0) for i in range(self.num_nodes)], dtype=torch.float).view(-1, 1)
        pagerank_feat = torch.tensor([pagerank.get(i, 0.0) for i in range(self.num_nodes)], dtype=torch.float).view(-1, 1)

        structural_x = torch.cat([degree_feat, clustering_feat, betweenness_feat, pagerank_feat], dim=1)
        structural_x = (structural_x - structural_x.mean(dim=0)) / (structural_x.std(dim=0) + 1e-6)

        # Community detection and one-hot encoding
        communities = list(greedy_modularity_communities(G_nx))
        self.community_map = {}
        for i, group in enumerate(communities):
            for node in group:
                self.community_map[node] = i
        community_ids = torch.tensor([self.community_map.get(i, 0) for i in range(self.num_nodes)], dtype=torch.long)
        community_onehot = F.one_hot(community_ids, num_classes=len(communities)).float()

        # Final node features
        final_x = torch.cat([structural_x, community_onehot], dim=1)
        self.x = final_x
        self.num_features = final_x.size(1)

        # Save edge_index for subgraph extraction
    ...
    def sample_negative_edges(self):
        ...
        edge_set = {tuple(sorted(edge)) for edge in self.positive_edges}
        while len(neg_edges) < len(self.positive_edges):
            i = torch.randint(0, num_nodes, (1,)).item()
            j = torch.randint(0, num_nodes, (1,)).item()
            if i == j or (min(i, j), max(i, j)) in edge_set:
                continue
    ...
    def __getitem__(self, idx):
        ...
        # Get subgraph and mask
        nodes, sub_edge_index, mapping, edge_mask = k_hop_subgraph(
            node_idx=node_pair,
        ...
        # Remove direct edge (if positive sample)
        if label.item() == 1:
            i, j = mapping[0].item(), mapping[1].item()
            mask = ~(((sub_edge_index[0] == i) & (sub_edge_index[1] == j)) |
                     ((sub_edge_index[0] == j) & (sub_edge_index[1] == i)))
            sub_edge_index = sub_edge_index[:, mask]

            # Skip if isolated
            if not ((sub_edge_index[0] == i).any() or (sub_edge_index[1] == i).any()):
                return self.__getitem__((idx + 1) % len(self))
            if not ((sub_edge_index[0] == j).any() or (sub_edge_index[1] == j).any()):
                return self.__getitem__((idx + 1) % len(self))

        # Gather node features
        x = self.x[nodes]
        batch = torch.zeros(x.size(0), dtype=torch.long)
        new_node_pair = mapping  # source/target in the subgraph
        ...
        data = Data(
            ...
            y=label,
            node_pair=new_node_pair.view(1, 2),
            batch=batch
        )
